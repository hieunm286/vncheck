...
export function GetSelectRow<T>({entities, selectedEntities, onSelectMany}: { entities: T[], selectedEntities: T[], onSelectMany: (entities: T[]) => void }) {
  return {
    mode: 'checkbox' as RowSelectionType,
    clickToSelect: true,
    hideSelectAll: false,
    selectionHeaderRenderer: () => {
      const isSelected = selectedEntities && selectedEntities.length > 0 && selectedEntities.length === entities.length;
    ...
    selectionRenderer: ({rowIndex}: { rowIndex: number }) => {
      const isSelected = selectedEntities && selectedEntities.some(entity => isEqual(entity, entities[rowIndex]));
      return (
        <SelectionCheckbox
          isSelected={isSelected}
          onChange={() => {
...
export function MasterTable<T>({
                                 ...
                                 onSelectMany,
                                 columns
                               }: ActionColumnProps<T> & { selectedEntities: T[], columns: ColumnDescription[], entities: T[], [T: string]: any }) {
