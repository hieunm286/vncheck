...
export function AgencyEditForm({saveAgency, agency, actionsLoading, onHide}: any) {
    const [administrativeDivision, setAdministrativeDivision] = useState({
        state: '',
        city: '',
        district: '',
        status: agency.status == 0,
        address: '',
    });

    const history = useHistory();

    const [agencyImage, setAgencyImage] = useState(null as unknown as string);
    const [imgPreview, setImgPreview] = useState(null as unknown as string);
    const refImage = useRef(null as unknown as string);
    ...
    const [shippingArr, setShippingArr] = useState([] as any[]);

    useEffect(() => {
        setAdministrativeDivision({
            state: agency.agency_addresses
                ? getCodeFromName(Object.values(STATE_LIST), agency.agency_addresses.state).toString()
                : '',
            city: getCodeFromName(Object.values(CITY_LIST), agency.agency_addresses?.city).toString(),
            district: getCodeFromName(Object.values(DISTRICT_LIST), agency.agency_addresses?.district).toString(),
            status: agency.status == 0,
    ...
    const getDataFromRow = (data: any, rowIndex: any) => {
    ...
    const saveNewData = (data: any, rowIndex: any) => {
    ...
    const switchAddress = (rowIndex: any) => {
                                            ...
                                            {imgPreview && (
                                                <div className="imagePreview">
                                                    ...
                                                    <span
                                                        className="close"
                                                        onClick={() => {
                                                            setAgencyImage(null as unknown as string);
