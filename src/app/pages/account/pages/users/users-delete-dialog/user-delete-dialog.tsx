...
export function UserDeleteDialog({id, show, onHide}: any) {
    // Users UI Context
    ...
    const {isLoading} = useSelector(
        (state: any) => ({isLoading: state.users.actionsLoading}),
        shallowEqual,
    );
    ...
    const deleteUser = () => {
        ...
        (dispatch(actions.deleteUser(id)) as any).then(() => {
        ...
        <Modal show={show} onHide={onHide} aria-labelledby="example-modal-sizes-title-lg">
            ...
            <Modal.Footer>
                <div>
                    ...
                    <> </>
                    <button type="button" onClick={deleteUser} className="btn btn-danger btn-elevate">
                        Delete
                    </button>
                </div>
            </Modal.Footer>
